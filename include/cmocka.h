...
#define cast_to_uintptr_type(value) ((uintptr_t)(value))
...
#define will_return(function, value) \
    _will_return(#function, __FILE__, __LINE__, cast_to_uintmax_type(value), 1)
#endif
...
#define will_return_count(function, value, count)                            \
    _will_return(#function, __FILE__, __LINE__, cast_to_uintmax_type(value), \
                 count)
#endif
 ...
 *
 */
void expect_check_count(#function, #parameter, #check_function,
                        const void* check_data, size_t count);
#else
#define expect_check_count(function, parameter, check_function, check_data,  \
                           count)                                            \
    _expect_check(#function, #parameter, __FILE__, __LINE__, check_function, \
 ...
 * @see check_expected().
 */
void expect_in_set_count(#function, #parameter, uintmax_t value_array[],
                         size_t count);
#else
 ...
 * @see check_expected().
 */
void expect_in_range(#function, #parameter, uintmax_t minimum,
                     uintmax_t maximum);
#else
 ...
 * @see check_expected().
 */
void expect_in_range_count(#function, #parameter, uintmax_t minimum,
                           uintmax_t maximum, size_t count);
#else
 ...
 * @see check_expected().
 */
void expect_not_in_range(#function, #parameter, uintmax_t minimum,
                         uintmax_t maximum);
#else
 ...
 * @see check_expected().
 */
void expect_not_in_range_count(#function, #parameter, uintmax_t minimum,
                               uintmax_t maximum, size_t count);
#else
 ...
 * @see check_expected().
 */
void expect_not_value_count(#function, #parameter, uintmax_t value,
                            size_t count);
#else
...
#define assert_true(c) \
    _assert_true(cast_to_uintmax_type(c), #c, __FILE__, __LINE__)
#endif
...
#define assert_return_code(rc, error) \
    _assert_return_code((rc), (error), #rc, __FILE__, __LINE__)
#endif
...
#define assert_non_null(c) \
    _assert_true(cast_ptr_to_uintmax_type(c), #c, __FILE__, __LINE__)
#endif
...
#define assert_ptr_equal(a, b)                      \
    _assert_uint_equal(cast_ptr_to_uintmax_type(a), \
                       cast_ptr_to_uintmax_type(b), __FILE__, __LINE__)
#endif
...
#define assert_int_equal(a, b)                                        \
    _assert_int_equal(cast_to_intmax_type(a), cast_to_intmax_type(b), \
                      __FILE__, __LINE__)
#endif
...
#define assert_uint_equal(a, b)                                          \
    _assert_uint_equal(cast_to_uintmax_type(a), cast_to_uintmax_type(b), \
                       __FILE__, __LINE__)
#endif
...
#define assert_int_not_equal(a, b)                                        \
    _assert_int_not_equal(cast_to_intmax_type(a), cast_to_intmax_type(b), \
                          __FILE__, __LINE__)
#endif
...
#define assert_uint_not_equal(a, b)                                          \
    _assert_uint_not_equal(cast_to_uintmax_type(a), cast_to_uintmax_type(b), \
                           __FILE__, __LINE__)
#endif
...
#define assert_float_not_equal(a, b, epsilon)                             \
    _assert_float_not_equal((float)a, (float)b, (float)epsilon, __FILE__, \
                            __LINE__)
#endif
...
#define assert_double_equal(a, b, epsilon)                                \
    _assert_double_equal((double)a, (double)b, (double)epsilon, __FILE__, \
                         __LINE__)
#endif
...
#define assert_double_not_equal(a, b, epsilon)                               \
    _assert_double_not_equal((float)a, (double)b, (double)epsilon, __FILE__, \
                             __LINE__)
#endif
...
#define assert_int_in_range(value, minimum, maximum)   \
    _assert_int_in_range(cast_to_intmax_type(value),   \
 ...
 * @param[in]  maximum  The maximum value allowed.
 */
void assert_uint_in_range(uintmax_t value, uintmax_t minimum,
                          uintmax_t maximum);
#else
#define assert_uint_in_range(value, minimum, maximum)   \
    _assert_uint_in_range(cast_to_intmax_type(value),   \
                          cast_to_intmax_type(minimum), \
                          cast_to_intmax_type(maximum), __FILE__, __LINE__)
#endif
...
#define assert_in_range(value, minimum, maximum)    \
    _assert_in_range(cast_to_uintmax_type(value),   \
...
#define assert_not_in_range(value, minimum, maximum)    \
    _assert_not_in_range(cast_to_uintmax_type(value),   \
                         cast_to_uintmax_type(minimum), \
                         cast_to_uintmax_type(maximum), __FILE__, __LINE__)
#endif
 ...
 * The test is considered passed if there are no leftover values, otherwise a
 * test failure is signaled. Calling stop() is especially useful in mocked
 * functions that do not return, e.g reset the CPU.
 */
void stop(void);
#else
#define stop() _stop()
...
void _expect_not_in_range(const char* const function,
                          ...
                          const int line, const uintmax_t minimum,
...
void _will_return(const char* const function_name, const char* const file,
                  const int line, const uintmax_t value, const int count);
void _assert_true(const uintmax_t result, const char* const expression,
                  const char* const file, const int line);
void _assert_return_code(const intmax_t result, const int32_t error,
                         const char* const expression, const char* const file,
                         const int line);
void _assert_float_equal(const float a, const float n, const float epsilon,
                         const char* const file, const int line);
void _assert_float_not_equal(const float a, const float n, const float epsilon,
                             const char* const file, const int line);
void _assert_double_equal(const double a, const double n, const double epsilon,
                          const char* const file, const int line);
void _assert_double_not_equal(const double a, const double n,
                              const double epsilon, const char* const file,
                              const int line);
void _assert_int_equal(const intmax_t a, const intmax_t b,
                       const char* const file, const int line);
void _assert_int_not_equal(const intmax_t a, const intmax_t b,
                           const char* const file, const int line);
void _assert_uint_equal(const uintmax_t a, const uintmax_t b,
                        const char* const file, const int line);
void _assert_uint_not_equal(const uintmax_t a, const uintmax_t b,
                            const char* const file, const int line);
...
void _assert_memory_not_equal(const void* const a, const void* const b,
                              const size_t size, const char* const file,
                              const int line);
void _assert_int_in_range(const intmax_t value, const intmax_t minimum,
                          const intmax_t maximum, const char* const file,
                          const int line);
void _assert_uint_in_range(const uintmax_t value, const uintmax_t minimum,
                           const uintmax_t maximum, const char* const file,
                           const int line);
void _assert_in_range(const uintmax_t value, const uintmax_t minimum,
                      const uintmax_t maximum, const char* const file,
                      const int line);
void _assert_not_in_range(const uintmax_t value, const uintmax_t minimum,
                          const uintmax_t maximum, const char* const file,
                          const int line);
void _assert_in_set(const uintmax_t value, const uintmax_t values[],
                    const size_t number_of_values, const char* const file,
                    const int line);
void _assert_not_in_set(const uintmax_t value, const uintmax_t values[],
                        const size_t number_of_values, const char* const file,
                        const int line);
